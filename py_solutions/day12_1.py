...
class CaveSystem:
    ...
    def search(self, current_cave, visited, path):
        ...
        for cave in current_cave.edges:
            ...
            if cave.type == "minor" and cave.name not in visited:
                new_path.append(cave.name)
                new_visited.add(cave.name)
                self.search(cave, new_visited, new_path)
...
def main():
    ...
    visited ={"start"}
    path = ["start"]
    caves.search(start, visited, path)
