...
def main():
    ...
    while synchronized_flash != True:
        ...
        for loc in mapped_octopi.keys():
            ...
            if mapped_octopi[loc].energy != 0:
                synchronized_flash = False

        if synchronized_flash == True:
            print(f"synchronized flash on step: {step}")

    print(f"count of flashes: {count}")
